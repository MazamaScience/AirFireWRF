...
For instance, you can build a plot with a Raster for a WRF variable, a 
vectorfield for WRF vector components, and layers for spatial points and 
polygons. Let's outline an example plot to showcase these different features.
...
To start with, let's load the WRFmet library and open the July 15, 2020 WRF file
(recent runs available [here](http://m2.airfire.org/PNW/)). This will give us 
access to read in meteorological data like temperature and wind velocity:
...
Now let's rasterize the WRF temperature and wind data. For this we must define a
grid, then use `wrf_createRaster()` to create RasterBricks for the specified 
variables. For temperature we will specify the 'T2' WRF variable, which gives us
a RasterBrick with a single layer. For wind velocity, we will specify the 'U10' 
and 'V10' WRF variables (North & East vector components) which will give us a 
RasterBrick with two layers:
...
```{r}
rasterVis::levelplot(tempRaster, main = 'Temperature (K)', margin = FALSE)
```

And for wind velocity, we can use the North and East vector components in
`vectorplot()` to make a vectorfield:
