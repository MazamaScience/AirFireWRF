...
2020 WRF file (recent runs available [here](http://m2.airfire.org/PNW/)). To do 
this, we must define a grid, then use `wrf_load()` to create RasterBricks for 
the specified variables. For temperature we will specify the 'T2' WRF variable
which gives us a RasterBrick with a single layer. For wind velocity, we will 
specify the 'U10' and 'V10' WRF variables (North & East vector components) which
will give us a RasterBrick with two layers:
