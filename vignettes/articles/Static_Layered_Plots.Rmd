...
data takes up quite a bit of space, so this article will load in a sample 
...
But for now, let's use a sample Raster already included in the package:
...
tempRaster <- raster::brick(PNW_Sample$TSK)
windRaster <- raster::brick(PNW_Sample$U10, PNW_Sample$V10)

# Extract the underlying grid these Rasters are using
extent <- raster::extent(tempRaster)
xlim <- c(round(extent@xmin), round(extent@xmax))
ylim <- c(round(extent@ymin), round(extent@ymax))
res <- round(mean(raster::res(tempRaster)), digits = 2)
```
